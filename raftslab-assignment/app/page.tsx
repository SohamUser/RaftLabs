  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      <div className="max-w-6xl mx-auto px-4 py-12">
        <section className="bg-card rounded-2xl p-8 md:p-12 shadow-sm border border-border">
          <div className="md:flex md:items-center md:justify-between gap-8">
            <div className="max-w-2xl">
              ...
              <div className="mt-6 flex items-center gap-3">
                <Link
                  href="/startups"
                  className="inline-flex items-center gap-2 bg-primary hover:bg-primary/90 text-primary-foreground px-5 py-2.5 rounded-full font-medium shadow"
                >
                  Browse startups
                </Link>
        ...
        <section className="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            ...
            { label: "Unique sectors", value: sectorCount },
            { label: "Curated list", value: "Static JSON" },
          ].map((item, i) => (
            <div
              key={i}
          ...
          <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            {featured.map((s) => (
              <Link
                ...
                className="group block rounded-xl bg-card border border-border p-4 hover:shadow-md transition"
              >
                <div className="flex items-center gap-4">
                  <div className="h-14 w-14 rounded-md bg-muted flex items-center justify-center border border-border overflow-hidden">
                    {s.logo ? (
                      <img
                        src={s.logo}
                        alt={`${s.name} logo`}
        ...
        <section className="mt-12 bg-card rounded-xl border border-border p-6 text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground max-w-xl mx-auto">
            Browse the full directory to find companies by sector, funding, and location.
          </p>
